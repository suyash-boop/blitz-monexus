...
async function main() {
  ...
  console.log(`Deploying PredictionGame contract to ${label}...\n`);
  ...
  const MIN_BET_AMOUNT = ethers.parseEther("0.01"); // 0.01 MON minimum bet
  const TREASURY_FEE = 300n; // 3%

  console.log("Min bet amount:", ethers.formatEther(MIN_BET_AMOUNT), currency);
  console.log("Treasury fee:", Number(TREASURY_FEE) / 100, "%");
  console.log("\nDeploying...");

  // Read artifact
  const artifact = await hre.artifacts.readArtifact("PredictionGame");

  // Encode constructor args
  const iface = new ethers.Interface(artifact.abi);
  const constructorData = iface.encodeDeploy([MIN_BET_AMOUNT, TREASURY_FEE]);
  ...
  console.log("\n========================================");
  console.log("SUCCESS! PredictionGame contract deployed!");
  console.log("========================================");
  console.log("\nContract address:", receipt.contractAddress);
  console.log("\nAdd this to your .env file:");
  console.log(`NEXT_PUBLIC_PREDICTION_GAME_ADDRESS=${receipt.contractAddress}`);
  console.log("\n========================================");
}
